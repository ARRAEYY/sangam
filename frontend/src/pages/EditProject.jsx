...
export default function EditProject() {
  ...
  useEffect(() => {
    api.getProject(id).then(project => {
      ...
      setDetailedDescription(project.description || "");
      setLookingFor(project.looking_for || "");
      setExpectations(project.expectations || "");
      setSelectedSkills(project.required_skills ? project.required_skills.map(s => s.name) : []);
      setTechStack(project.tech_stack || []);
      setTeamMembers(project.members ? project.members.filter(m => m.user_id !== user?.id).map(m => ({ user_id: m.user_id, role: m.role, role_category: m.role_category, full_name: m.user?.full_name, avatar_url: m.user?.avatar_url })) : []);
  ...
  async function submit(event) {
    ...
    try {
      ...
      await api.editProject(id, payload);
    ...
    <div className="page-stack create-page w-full max-w-[1200px] mx-auto pb-20">
      ...
      <section className="create-intro reveal-in delay-1">
        <div>
          ...
          <h1>Edit your<br /><em className="not-italic text-[#7f1d3b]">project.</em></h1>
          <p>Update the details to keep everyone on the same page.</p>
        </div>
        ...
        <form className="create-form" onSubmit={submit}>
          ...
          <div className="form-actions mt-8 pt-8 border-t border-slate-100">
            ...
            <button type="submit" className="button button-primary" disabled={submitting}>
              {submitting ? 'Saving...' : 'Save Changes'} <ArrowRight size={16} />
            </button>
          </div>
        </form>
