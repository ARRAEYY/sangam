    ...
    <aside className="sticky top-24 hidden h-fit shrink-0 md:block">
      <div className="card flex w-[72px] flex-col items-center gap-1 py-4">
        {items.map(({ to, label, icon: Icon }) => {
          ...
          return (
            <Link key={to} to={to} title={label} className={`icon-nav-btn ${active ? 'active' : ''}`}>
        ...
        {user && (
          <>
            ...
            <button onClick={logout} title="Log out" className="icon-nav-btn hover:bg-red-50 hover:text-red-500">
