...
export default function MobileBottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 block border-t border-slate-200/80 bg-white/95 px-2 py-1 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] backdrop-blur-md md:hidden">
      <nav className="mx-auto flex max-w-md items-center justify-between">
        {navItems.map(({ to, label, icon: Icon, isAction, badge }) => {
          ...
          if (isAction) {
            return (
              <Link
                key={to}
                to={user ? to : '/auth'}
                className="group relative -mt-5 flex flex-col items-center px-2"
              >
